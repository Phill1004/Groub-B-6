...
public class Queen extends Piece{
	...
	public boolean checkMove(int[] moveFromReq, int[] moveToReq, String plyColor, boolean testKing) {
		...
		Square toSquare = gameBoard.board[moveToY][moveToX];
		...
		if(type == "diagonal"){
			...
			for(int diagMoveAway = 1; diagMoveAway <= moveDistance; diagMoveAway++){
				...
				if(direction == "topRite"){
					testSquare = gameBoard.board[moveFromY - diagMoveAway][moveFromX + diagMoveAway];
				}
			...
			if((direction == "rite") || (direction == "left")){
				...
				for(int displace = 1; displace <= displaceMax; displace++){ //looping through squares on the rooks path
					if(direction == "rite"){
						testSquare = gameBoard.board[moveFromY][moveFromX + displace];
				...
				for(int displace = 1; displace <= displaceMax; displace++){ //looping through squares on the rooks path	
					...
					if(direction == "top"){
						testSquare = gameBoard.board[moveFromY - displace][moveFromX];
