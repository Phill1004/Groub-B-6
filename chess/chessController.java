...
class chessController {
	...
	chessController(String nameIn, String colorIn){
		name = nameIn;
		color = colorIn;
	}
	...
	//the user will enter a char but it need to be a num for program
		...
		for(int i = 0; i < gameBoard.SIDE_LETTERS.length; i++){
			
			if(gameBoard.SIDE_LETTERS[i] == charIn){
				numOut = i;
			}
		}
		return numOut;
					...
					if(!Character.isDigit(moveIn.charAt(0)) && Character.isDigit(moveIn.charAt(1))){
						...
						if((x = convertCharToNum(Character.toUpperCase(moveIn.charAt(0)))) != -1){
							if((y = convertCharNumtoNum(moveIn.charAt(1))) != -1){
								...
								int tempArray[] = {x, y};
								if(runNum == 1){
									if(gameBoard.board[y][x].getType() == "blank" || gameBoard.board[y][x].getColor() != color){ 
